...
install_gnparser_bin = function(exec) {
   ...
   dirs <- bin_paths()
   for (destdir in dirs) {
      if (dir_writable(destdir)) {
         base::dir.create(destdir, showWarnings = FALSE)
         success <- base::file.copy(exec, destdir, overwrite = TRUE)
         if (success) break
      }
   }
   if (!success) stop(
   ...
   base::message('gnparser has been installed to ', base::normalizePath(destdir))
}
...
dir_writable <- function(x) utils::file_test("-w", x)
